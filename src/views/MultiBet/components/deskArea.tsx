...
export const BetTop: FC<Props> = ({
  ...
  const addChips = () => {
    Communicator.action("multi_addChips", { category: en, desk, store });
  };
  ...
  return (
    <DeskBlock
      key={en}
      onClick={addChips}
      // onClick={() => addChipsTodDesk({ category: en, store })}
      className={en}
...
export const BetBottom: FC<Props> = ({
  ...
  const addChips = () => {
    Communicator.action("multi_addChips", { category: en.toLocaleLowerCase(), desk, store });
  };
